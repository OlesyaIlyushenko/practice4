...
func handleConnection(conn net.Conn) {
	...
	for scanner.Scan() {
		...
		if args[0] == "1" {
			Statistic(args[1], args[2], args[3])
		} else if args[0] == "2" {
			...
			jsonData := Report(ditalization, JsonFile)
...
func Statistic(url, shortURL, ip string) {
	...
	newConnect.PID = generateParentID(connections, url)
	connections = append(connections, newConnect)

	err = writeStatistic(connections)
	if err != nil {
		fmt.Println("Ошибка записи в файл:", err)
		return
	}
...
func readStatistic() ([]Connection, error) {
	var Connections []Connection
...
func writeStatistic(workers []Connection) error {
...
func generateParentID(Connections []Connection, url string) int {
	PID := 0
	for _, connect := range Connections {
		if connect.URL == url {
			PID = connect.ID
...
func Report(ditalization []string, connections []Connection) map[string]interface{} {
